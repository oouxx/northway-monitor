...
onMounted(() => {
  // isEE.value = !!(window.require && window.require('electron'))
  showHost.value = isEE.value || route.query.desktop
  domain.value = localStorage.getItem('domain') || ''
})
async function login() {
  if (domain.value) {
    window.baseURL = `${isEE.value ? 'https:' : location.protocol}` + '//' + domain.value
    window.remoteHost = domain.value
  ...
  await loginApi.login(userForm.name, userForm.pass)
  ...
  router.push({
    ...
    query: { auth: window.btoa(`${userForm.name}:${userForm.pass}`) }
  })
}
