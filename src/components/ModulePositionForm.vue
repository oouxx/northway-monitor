      ...
      <el-form ref="positionInfo" :model="form" label-width="70px" width="200px">
        <el-form-item label="合约代码" prop="contractId">
          <el-select v-model="form.contractId">
            <el-option v-for="c in contractOptions" :key="c.value" :value="c.value" :label="c.name"></el-option>
          </el-select>
        </el-form-item>
...
let directionComb = ref('')
watch(directionComb, (val) => {
  form.direction = val[0]
  form.offsetFlag = val[1]
})
