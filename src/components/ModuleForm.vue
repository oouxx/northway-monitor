<template>
  <el-dialog :title="readOnly ? '查看' : isUpdateMode ? '修改' : '新增'" :model-value="visible" class="module-dialog"
    :close-on-click-modal="readOnly" :width="`${isMobile ? '80%' : '540px'}`" @close="close">
            ...
            <div v-show="activeIndex === '3'">
              <el-form-item label="绑定账号">
                <el-select v-model="choseAccounts" placeholder="请选择账户" multiple :disabled="readOnly"
                  ...
                  <el-option v-for="(acc, i) in accountOptions" :label="acc.gatewayId" :value="acc"
                    :key="i"></el-option>
                </el-select>
              </el-form-item>
              <div v-for="(acc, i) in form.moduleAccountSettingsDescription" :key="i">
                ...
                <el-form-item label="关联合约">
                  <el-select class='bindContractSelector'
                    ...
                    <el-option v-for="(item, i) in bindedContractsOptions[i]" :value="item" :label="item.name"
                      :key="i" />
                  </el-select>
                </el-form-item>
              </div>
